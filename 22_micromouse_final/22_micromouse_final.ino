// ╔══════════════════════════════════════════════════════════════════╗
// ║  22_micromouse_final                                             ║
// ║  Flood-fill micromouse. Maze: 10 cells across x 5 ahead, 18 cm   ║
// ║  cells, start in the left corner. Goal: the 2x2 room (4 cells,   ║
// ║  36 x 36 cm) in the far right corner.                            ║
...
// ║                                                                  ║
// ║  If it goes wrong (crash, stuck, a bad turn):                    ║
// ║    it stops by itself, or press the button. The LED blinks twice ║
// ║    and stays on: the map is put back to how it was when the      ║
// ║    robot last stopped in a cell, so nothing from the bad move    ║
// ║    is kept. Put the robot back in THAT cell, in the middle,      ║
// ║    facing the way it faced when it stopped there, and press:     ║
// ║    it reads the walls again and redoes the move. If the same     ║
// ║    move goes wrong twice, it takes it as blocked and goes        ║
// ║    another way. Long press instead = end the run (map kept).     ║
// ║                                                                  ║
// ║  LED: 5 blinks = reached the goal, 3 blinks = map cleared,       ║
// ║       2 blinks then on = paused, waiting for you.                ║
// ║                                                                  ║
// ║  USB (Serial Monitor, 115200): the map is printed at every pause ║
// ║  and at the end of every run; send 'm' any time the robot is     ║
// ║  waiting to print it again. Each cell is one byte, see printMap. ║
// ║                                                                  ║
// ║  Flood fill with turn costs on (cell, heading): a turn costs     ║
// ║  more than going straight on, so it keeps going forward unless   ║
// ║  turning really saves distance. Search: to the goal, then back   ║
// ║  to the start, unseen walls counted as open (as mazerunner-core);║
// ║  speed run on seen walls only.                                   ║
// ║                                                                  ║
...
// ║   * Front: stops at the right spot; under 20 mm = crash, pause.  ║
// ║   * Every stop: walls read; a clear-cut reading that disagrees   ║
// ║     with the map (read twice) corrects it; open corridor 1-2     ║
...
#define SIDE_SURE_WALL_MM   90     // side reading below this: certainly a wall
#define SIDE_SURE_OPEN_MM   170    // side reading above this: certainly no wall
#define FRONT_SURE_MARGIN_MM 55    // front reading this far either side of the threshold: certain
#define BUTTON_PAUSE_COUNT  5      // button must read pressed this many times in a row to pause a run
#define LONG_PRESS_MS       1000
#define DOUBLE_CLICK_MS     400    // second press within this = double click
#define EEPROM_MAGIC        (0x70 ^ MAZE_SHORT ^ (MAZE_LONG << 3) ^ GOAL_SIZE)

#define DRIVE_STALL 0
#define DRIVE_DONE  1
#define DRIVE_WALL  2
#define DRIVE_CRASH 3   // front closer than FRONT_EMERGENCY_MM: something went wrong
...
bool waitingForButton = false;    // paused, waiting to be put back (not "driving" for the reset check)
uint8_t buttonLowCount = 0;       // sense() calls in a row with the button down (debounce)
...
// This section is compiled on a PC by tests/micromouse_22_sim.sh and run on random mazes.
...
#define MAX_CORRECTIONS 6  // clear-cut readings allowed to change the map per run (a limit, in case it is lost)
#define MAX_RUN     17  // most cells driven in one straight

// Flood costs. A 90° arc (settle, check, arc) takes longer than driving a cell straight on,
// and a U-turn longer still, so the flood counts them: the robot only turns when that
// really saves distance.
#define COST_CELL    2
#define COST_TURN    3
#define COST_UTURN   6
...
uint8_t corrections = 0;       // map changes made this run, see recordWalls()
...
// Extra cost of turning from heading `from` to heading `to` before driving on.
uint8_t turnCost(uint8_t from, uint8_t to) {
  uint8_t t = (to - from) & 3;
  return t == 0 ? 0 : t == 2 ? COST_UTURN : COST_TURN;
}

// Flood fill with turn costs. cost[x][y][h] = cheapest way to a target from cell (x, y) when
// facing h: each cell driven costs COST_CELL, plus the turn made before it. Worked outward from
// the targets one cost level at a time (like a plain flood fill, which is this with no turn
// costs), so it needs no queue.
void flood(bool pessimistic) {
  memset(cost, 255, sizeof(cost));
  for (uint8_t i = 0; i < tgN; i++)
    for (uint8_t h = 0; h < 4; h++) cost[tgX[i]][tgY[i]][h] = 0;
  uint8_t highest = 0;
  for (uint8_t level = 0; level <= highest && level < 254; level++) {
    for (int8_t x = 0; x < MAZE_W; x++) {
      for (int8_t y = 0; y < MAZE_H; y++) {
        for (uint8_t h = 0; h < 4; h++) {
          if (cost[x][y][h] != level) continue;
          // Arrived in (x, y) facing h: came straight from the cell behind, facing h0 there.
          int8_t px = x - DX[h], py = y - DY[h];
          if (!inMaze(px, py) || blocked(px, py, h, pessimistic)) continue;
          for (uint8_t h0 = 0; h0 < 4; h0++) {
            uint16_t c = level + COST_CELL + turnCost(h0, h);
            if (c < cost[px][py][h0] && c < 255) {
              cost[px][py][h0] = c;
...
// Cheapest way to a target from (x, y) facing any way (255 = none).
uint8_t cellCost(int8_t x, int8_t y) {
  uint8_t c = 255;
  for (uint8_t h = 0; h < 4; h++) if (cost[x][y][h] < c) c = cost[x][y][h];
  return c;
}

// Best heading to drive out of (x, y) when facing f: the cheapest turn + cell + onward cost.
// Ties: ahead, then right, left, back. 255 = no way out.
uint8_t bestDir(int8_t x, int8_t y, uint8_t f, bool pessimistic) {
  static const uint8_t order[4] = { 0, 1, 3, 2 };
  uint8_t best = 255;
  uint16_t bestCost = 255;
  for (uint8_t i = 0; i < 4; i++) {
    uint8_t d = (f + order[i]) & 3;
    if (blocked(x, y, d, pessimistic)) continue;
    int8_t nx = x + DX[d], ny = y + DY[d];
    if (cost[nx][ny][d] == 255) continue;
    uint16_t c = turnCost(f, d) + COST_CELL + cost[nx][ny][d];
...
uint8_t runLength(int8_t x, int8_t y, uint8_t d, bool pessimistic) {
  uint8_t n = 1;
  x += DX[d]; y += DY[d];
  while (n < MAX_RUN && cost[x][y][d] != 0 && bestDir(x, y, d, pessimistic) == d) {
    x += DX[d]; y += DY[d]; n++;
  }
  return n;
}

// Speed-run cost from the start to the goal on seen walls (255 = no route yet).
uint8_t knownPathLength() {
  targetGoal();
  flood(true);
  return cost[startX][0][0];
}
...
uint8_t driveTo(bool pessimistic, bool multiCell, uint8_t &dir) {
  flood(pessimistic);
  if (cost[posX][posY][facing] == 0) return 0;
  dir = bestDir(posX, posY, facing, pessimistic);
  if (dir == 255) { noRoute = true; return 0; }
...
// The same three walls as the map has them (bit set = wall), and which of them it has seen.
uint8_t mapWalls() {
  uint8_t w = 0;
...
uint8_t seenWalls() {
  uint8_t w = 0;
  for (uint8_t i = 0; i < 3; i++) if (maze[posX][posY] & (0x10 << ((facing + 3 + i) & 3))) w |= 1 << i;
  return w;
}

// Records the three walls the ToFs see from this cell's decision point. A wall is written the
// first time it is seen. After that only a clear-cut reading (bit set in `sure`, and already
// confirmed by a second reading) may change it, at most MAX_CORRECTIONS times a run: a
// borderline reading never changes the map, so it can't flip back and forth.
void recordWalls(uint8_t walls, uint8_t sure) {
...
// The front ToF sees open corridor `cellsOpen` cells beyond this one (0, 1 or 2): those walls,
// if not seen yet, are open.
void recordCorridor(uint8_t cellsOpen) {
  int8_t x = posX, y = posY;
  for (uint8_t k = 0; k < cellsOpen; k++) {
    x += DX[facing]; y += DY[facing];
    if (!inMaze(x, y)) return;
    if (!(maze[x][y] & (0x10 << facing))) putWall(x, y, facing, false);
  }
}
...
void recoverRoute() {
  tgN = 0;
  addTarget(posX, posY);
  flood(false);  // cellCost != 255: cells the robot can reach
  bool any = false;
...
// The button only sets a flag; runStep() pauses as soon as the current move ends. It must be
// down for BUTTON_PAUSE_COUNT calls in a row (~15 ms), so motor noise on the wire can't pause.
void sense() {
  updateGyro();
  updateToF();
  if (runState == ST_RUN && digitalRead(START_BUTTON) == LOW) {
...
// Something is really close in front: two readings in a row, so one bad reading can't stop it.
...
bool executeTurn(char move) {
  ...
  if (atCentre && !(move == 'U' && tofF < FRONT_WALL_THRESHOLD_MM)) {
    ...
    // (spinSafely shuffles so the corners clear). It ends in the middle of the cell again,
    // facing the new way.
...
uint8_t moveCells(uint8_t numCells, int maxPwm) {
  ...
  lastDriveCrashed = (r == DRIVE_CRASH);
  backToWall = false;
  return k;
}
...
// them in the map (see recordWalls). A clear-cut reading that disagrees with the map is read a
// second time; only if both readings are clear-cut and agree does it change the map.
// The standing reading is also used, with no extra moves:
//  * the front ToF looking down an open corridor marks the walls 1-2 cells ahead as open,
...
void senseWallsHere() {
  ...
  else if ((walls ^ mapWalls()) & seenWalls() & sure) {
    uint8_t walls2 = readWalls(l2, f2, r2);
    sure &= sureWalls(l2, f2, r2) & ~(walls ^ walls2);  // clear-cut both times, and the same
  }
  recordWalls(walls, sure);
  if (!(walls & 2)) {  // open ahead: how far does the corridor clearly go on?
    float farWall = FRONT_WALL_THRESHOLD_MM - carryMm + FRONT_SURE_MARGIN_MM;  // clear of this cell's front wall
    recordCorridor(f > farWall + 2 * CELL_MM ? 2 : f > farWall + CELL_MM ? 1 : 0);
  }
...
void saveMaze() {
  ...
  EEPROM.update(addr++, runState == ST_RUN && !waitingForButton);
}
...
// --- MISTAKES: PAUSE, PUT BACK, REDO ---
// At every stop the robot remembers the whole map and where it is ("the last good stop").
// If a move goes wrong (turn failed, stuck, front closer than 20 mm) or the button is pressed,
// everything since that stop is thrown away, so a crash never puts anything in the map.
// Then it waits to be put back at that stop and redoes the move. The same move going wrong
...
bool pauseAndRedo(bool blame) {
  ...
  int b;
  do { b = readButton(); } while (b == 0);  // readButton turns the LED off
...
// One move of a run (like searchStep in 14): read the walls, plan, turn, drive. Anything that
// goes wrong pauses the run (see pauseAndRedo).
void runStep() {
  rememberStop();
  ...
  if (phase != PH_FAST) saveMaze();  // every cell, so a reset or power cut loses nothing
}
...
void loop() {
  ...
  sense();
  if (Serial.available() && Serial.read() == 'm') printMap();
  int b = readButton();
  if (b == 3) {
    clearMap();
    blink(3);
