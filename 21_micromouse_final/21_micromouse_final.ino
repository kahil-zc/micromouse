...
// This section is compiled on a PC by tests/micromouse_21_sim.sh and run on random mazes.
...
// The button only sets a flag; runStep() pauses as soon as the current move ends.
void sense() {
  updateGyro();
  updateToF();
  if (runState == ST_RUN && digitalRead(START_BUTTON) == LOW) pauseRequested = true;
}
