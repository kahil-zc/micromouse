...
// ║  Driving and turning are exactly those of 14_floodfill_final.    ║
// ║                                                                  ║
...
// ║                                                                  ║
// ║  Exploring (and driving home) goes one cell at a time, like 14:  ║
// ║  stop at the decision point of every cell, read the walls, plan. ║
// ║  It always heads for the nearest unmapped cell, straight ahead   ║
// ║  first, and never goes back just to look at a mapped cell.       ║
// ║                                                                  ║
...
// ║   * Every stop: the walls are read again, even in mapped cells.  ║
// ║     A reading that disagrees with the map is read a second time  ║
// ║     and, if both agree, the map is corrected.                    ║
// ║   * Every stop: a wall one cell ahead fixes the position along   ║
// ║     the corridor, and two side walls tune the centred side       ║
// ║     reading. No extra moves for either.                          ║
// ║   * Before a turn: pull up to the front wall if there is one.    ║
// ║   * Exploring ends with no goal: walls that may be shutting off  ║
// ║     part of the maze are driven to and read again.               ║
// ║                                                                  ║
...
// Size of the real maze in cells, either way round. Only used to decide which walls are worth
// reading again if exploring ends without finding the goal.
#define MAZE_SHORT   5
#define MAZE_LONG   10
...
#define EEPROM_MAGIC        (0xE0 ^ MAZE_W ^ (MAZE_H << 4))
...
// This section is compiled on a PC by tests/micromouse_17_sim.sh and run on random mazes.
...
#define MAX_RECHECKS 6  // walls read again when exploring ends without finding the goal
...
bool stopRequested = false;    // the button was pressed to stop exploring
uint16_t newCells = 0;         // cells mapped in the current phase

int8_t tgX[MAX_TARGETS], tgY[MAX_TARGETS];
uint8_t tgN = 0;
uint16_t rcWall[MAX_RECHECKS];  // walls already read again, as (cell index) * 4 + direction
uint8_t rechecks = 0;

bool inMaze(int8_t x, int8_t y) { return x >= 0 && y >= 0 && x < MAZE_W && y < MAZE_H; }
bool isStart(int8_t x, int8_t y) { return x == startX && y == 0; }
bool isGoal(int8_t x, int8_t y) { return goalKnown && x >= goalX && x <= goalX + 1 && y >= goalY && y <= goalY + 1; }
bool visited(int8_t x, int8_t y) { return (maze[x][y] & 0xF0) == 0xF0; }  // all four walls known
...
// pessimistic: unknown walls count as walls (speed run); otherwise as open (exploring).
...
// middle of the goal, 2 = no wall touches it and all four walls are known, so it IS the goal.
uint8_t postState(int8_t px, int8_t py) {
...
// Looks for the goal post; if there are several, the one closest to the middle wins.
void findGoal() {
  if (goalKnown) return;
  int best = 1000;
  for (int8_t px = 0; px < MAZE_W - 1; px++)
    for (int8_t py = 0; py < MAZE_H - 1; py++) {
      int d = abs(2 * px + 2 - MAZE_W) + abs(2 * py + 2 - MAZE_H);
      if (postState(px, py) == 2 && d < best) { best = d; goalX = px; goalY = py; }
    }
  if (best < 1000) goalKnown = true;
}

// A corrected wall can show the goal room was wrong: forget it, and look again.
void recheckGoal() {
  if (goalKnown && postState(goalX, goalY) != 2) goalKnown = false;
  findGoal();
}

// Target while exploring: the nearest unmapped cell the robot can reach (ties: the one most
// straight ahead). Keeps the current one until it is mapped, so the robot doesn't dither.
// False when every reachable cell is mapped.
bool exploreTargets() {
  tgN = 0;
  addTarget(posX, posY);
  flood(false);
  bool keep = exploreX >= 0 && !visited(exploreX, exploreY) && dist[exploreX][exploreY] != 255;
  if (!keep) {
    int bestD = 255, bestAhead = -1000;
    exploreX = -1;
    for (int8_t x = 0; x < MAZE_W; x++) {
      for (int8_t y = 0; y < MAZE_H; y++) {
        if (visited(x, y) || dist[x][y] == 255) continue;
        int ahead = (x - posX) * DX[facing] + (y - posY) * DY[facing];
        if (dist[x][y] < bestD || (dist[x][y] == bestD && ahead > bestAhead)) {
          bestD = dist[x][y]; bestAhead = ahead; exploreX = x; exploreY = y;
        }
      }
    }
    if (exploreX < 0) return false;
  }
  tgN = 0;
  addTarget(exploreX, exploreY);
  return true;
}

// Exploring is over and no goal room was found, so a misread wall may be shutting it off: a wall
// between a mapped cell and an unmapped cell that must be part of the maze (its size is known,
// and the mapped cells may already show which way round it is). Takes the nearest such wall and
// forgets it in the mapped cell, so the robot drives there and reads it again. The unmapped cell
// keeps the wall, and blocked() respects it, so the robot never tries to drive through it.
// Needs dist from exploreTargets() (distance from the robot). False = nothing worth reading again.
bool recheckSuspect() {
  if (rechecks >= MAX_RECHECKS) return false;
  int8_t maxA = 0, maxY = 0;  // how far the mapped cells reach across and ahead
  for (int8_t x = 0; x < MAZE_W; x++)
    for (int8_t y = 0; y < MAZE_H; y++)
      if (visited(x, y)) {
        if (abs(x - startX) > maxA) maxA = abs(x - startX);
        if (y > maxY) maxY = y;
      }
  uint8_t best = 255;
  uint16_t pick = 0;
  for (int8_t x = 0; x < MAZE_W; x++) {
    for (int8_t y = 0; y < MAZE_H; y++) {
      if (!visited(x, y) || dist[x][y] >= best) continue;
      for (uint8_t d = 0; d < 4; d++) {
        int8_t nx = x + DX[d], ny = y + DY[d];
        if (!(maze[x][y] & (1 << d)) || !inMaze(nx, ny) || visited(nx, ny)) continue;
        if (x == posX && y == posY && d == ((facing + 2) & 3)) continue;  // behind the robot: never read
        int8_t a = abs(nx - startX);
        bool across = a < MAZE_LONG && ny < MAZE_SHORT;  // fits with the long side across
        bool ahead = a < MAZE_SHORT && ny < MAZE_LONG;   // fits with the long side ahead
        if (!(maxY >= MAZE_SHORT ? ahead : maxA >= MAZE_SHORT ? across : across || ahead)) continue;
        uint16_t id = (x * MAZE_H + y) * 4 + d;
        bool done = false;
        for (uint8_t i = 0; i < rechecks; i++) done |= rcWall[i] == id;
        if (!done) { best = dist[x][y]; pick = id; }
      }
    }
  }
  if (best == 255) return false;
  rcWall[rechecks++] = pick;
  uint8_t d = pick & 3, c = pick >> 2;
  maze[c / MAZE_H][c % MAZE_H] &= ~((1 << d) | (0x10 << d));  // that cell is unmapped again
  exploreX = -1;
  return true;
}

// Shortest start-to-goal length on known walls (255 = no known path).
uint8_t knownPathLength() {
  if (!goalKnown) return 255;
  targetGoal();
  flood(true);
...
// route (noRoute set). Only the speed run drives several cells in one go; everything else goes
// one cell at a time, stopping in each cell to read the walls (the 14_floodfill_final way).
...
uint8_t plan(uint8_t &dir) {
  ...
  switch (phase) {
    case PH_EXPLORE:
      ...
      if (!exploreTargets()) {  // nowhere new left and no goal: read a suspect wall again
        if (!recheckSuspect() || !exploreTargets()) { phase = PH_HOME; return 0; }
      }
      return driveTo(false, false, dir);
    case PH_IMPROVE:
      recheckGoal();
      if (!goalKnown) { phase = PH_EXPLORE; exploreX = -1; return 0; }  // goal room was misread
      if (newCells >= MAX_IMPROVE_CELLS || !improveTargets()) { phase = PH_HOME; return 0; }
      return driveTo(false, false, dir);
    case PH_HOME: {
      bool hadGoal = goalKnown;
      recheckGoal();
      if (hadGoal && !goalKnown && !stopRequested) { phase = PH_EXPLORE; exploreX = -1; return 0; }  // it was misread
    ...
    case PH_FAST:
      ...
      if (noRoute) { noRoute = false; phase = PH_EXPLORE; }  // map incomplete: explore instead
      return n;
    case PH_RETURN:
      if (isStart(posX, posY)) { phase = PH_DONE; return 0; }
      targetStart();
...
// The same three walls as the map has them.
uint8_t mapWalls() {
  uint8_t w = 0;
  for (uint8_t i = 0; i < 3; i++) if (maze[posX][posY] & (1 << ((facing + 3 + i) & 3))) w |= 1 << i;
  return w;
}
...
void recordWalls(uint8_t walls) {
  ...
  for (uint8_t i = 0; i < 3; i++) putWall(posX, posY, (facing + 3 + i) & 3, walls & (1 << i));
}
...
void initToF(VL53L0X &sensor, uint8_t xshutPin, uint8_t addr) {
  ...
  if (sensor.init()) {
    ...
    Serial.print(F("ToF NOT FOUND on XSHUT pin ")); Serial.println(xshutPin);
  }
}
...
void noteResult(bool ok) {
  ...
  if (failCount >= MAX_FAILS) {
    Serial.println(F("TOO MANY FAILURES - put me in the start cell"));
...
void edgeUpdate(SideEdge &e, int tof, float done, float &corrMm, float startAxleMm, char side) {
  ...
  if (fabs(c) < EDGE_MAX_CORR_MM) {
    corrMm += c;
    Serial.print(side); Serial.print(F(" edge fix mm: ")); Serial.println((int)round(c));
  }
}
...
void fitUpdate(WallFit &f, int tof, float x, int sideSign) {
  ...
  if (den > 1) {
    ...
    if (fabs(err) < 8) {  // larger means a bad fit (post, open cell), not drift
      ...
      Serial.print(F("Gyro fix x0.1 deg: ")); Serial.println((int)round(-10 * step));
    }
  }
  f.n = 0;
}
...
void shiftSideways(float mm, float frontGapMm) {
  ...
  Serial.print(F("Shuffle sideways mm: ")); Serial.println((int)round(mm));
...
// One standing reading of the three walls (bits, see mapWalls). The front threshold allows for
// the robot not being exactly on the decision point.
...
// Reads the three walls of the current cell from its decision point and prints them. Done in
// every cell, mapped or not. A reading that disagrees with the map is read a second time, and a
// wall changes only when both readings agree, so one bad reading never changes the map.
...
void senseWallsHere() {
  ...
  if (!visited(posX, posY)) {
    ...
    uint8_t same = ~(walls ^ readWalls(l2, f2, r2));
    walls = (walls & same) | (mapWalls() & ~same);
    if (walls != mapWalls()) Serial.println(F("MAP CORRECTED"));
  }
  bool shifted = !cornerKnown;
  recordWalls(walls);
  if (shifted && startX != 0) Serial.println(F("Started in RIGHT corner"));
  ...
  Serial.print(' '); Serial.print("NESW"[facing]);
  Serial.print(F("  L ")); Serial.print((int)l); Serial.print(F(" F ")); Serial.print((int)f);
  Serial.print(F(" R ")); Serial.print((int)r); Serial.print(F("  walls "));
  for (uint8_t i = 0; i < 3; i++) Serial.print((walls & (1 << i)) ? "LFR"[i] : '-');
  Serial.print(F("  pos ")); Serial.println((int)carryMm);
}
...
void printMaze() {
  ...
  for (int x = 0; x < MAZE_W; x++) Serial.print(F("+---"));
  Serial.println('+');
}

// Prints what the map says about the goal and the speed-run path.
void reportPath() {
  if (!goalKnown) { Serial.println(F("No goal yet")); return; }
  Serial.print(F("Goal room at (")); Serial.print(goalX); Serial.print(','); Serial.print(goalY);
  Serial.print(F(")  path: "));
  uint8_t n = knownPathLength();
  if (n == 255) Serial.println(F("none"));
  else { Serial.print(n); Serial.println(F(" cells")); }
}

bool speedRunReady() { return knownPathLength() != 255; }

void printPrompt() {
  Serial.println(speedRunReady() ? F("Press = SPEED RUN, long = explore, double = clear map")
                                 : F("Press = EXPLORE, double = clear map"));
}
...
void announcePhase(uint8_t from) {
  if (from == PH_EXPLORE && phase == PH_IMPROVE) {
    ...
    Serial.println(F("\nAll mapped, no goal - going home"));
  } else if (from == PH_IMPROVE && phase == PH_HOME) {
    Serial.println(newCells >= MAX_IMPROVE_CELLS ? F("Limit reached - going home")
                                                 : F("Shortest path proven - going home"));
    saveMaze();
  } else if (from == PH_FAST && phase == PH_RETURN) {
    Serial.println(F("\n*** SPEED RUN DONE ***"));
    blink(5);
  } else if (from == PH_FAST && phase == PH_EXPLORE) {
    Serial.println(F("No mapped path - exploring"));
  } else if (phase == PH_EXPLORE) {
    Serial.println(F("Goal was misread - exploring"));
  }
...
void runStep() {
  ...
  uint8_t n = plan(dir);
  if (noRoute) {
    // Misread walls have closed every route. Start the map again and keep going.
    Serial.println(F("No route - map cleared"));
    initMaze();
    resetStart();
    putWall(startX, 0, 2, true);
    if (phase == PH_IMPROVE) phase = PH_EXPLORE;  // the goal was forgotten with the map
    return;
  }
  ...
  advance(dir, done);
  if (done < n && !lastDriveStalled) {  // the front ToF stopped it at a wall the map didn't have
    putWall(posX, posY, dir, true);
    Serial.println(F("Front ToF: new wall"));
  }
}
