...
bool inMaze(int x, int y) { return x >= 0 && y >= 0 && x < MAZE_W && y < MAZE_H; }
bool isStart(int x, int y) { return x == startX && y == 0; }
bool isGoal(int x, int y) { return goalKnown && x >= goalX && x <= goalX + 1 && y >= goalY && y <= goalY + 1; }
bool visited(int x, int y) { return (maze[x][y] & 0xF0) == 0xF0; }  // all four walls known
...
void initMaze() {
  ...
  for (int x = 0; x < MAZE_W; x++) {
    putWall(x, 0, 2, true, false);
    putWall(x, MAZE_H - 1, 0, true, false);
  }
  for (int y = 0; y < MAZE_H; y++) {
    putWall(0, y, 3, true, false);
    putWall(MAZE_W - 1, y, 1, true, false);
  }
  goalKnown = false;
  exploreX = exploreY = -1;
}
...
void findGoal() {
  if (goalKnown) return;
  int best = 1000;
  for (int px = 0; px < MAZE_W - 1; px++)
    for (int py = 0; py < MAZE_H - 1; py++) {
...
void finishAtHome() {
  ...
  printMaze();
  reportPath();
  Serial.println(speedRunReady() ? F("\nHOME. Press = SPEED RUN, long press = explore again.")
                                 : F("\nHOME. No goal found - press = explore again."));
  runState = ST_WAIT;
}
...
void announcePhase(uint8_t from) {
  if (from == PH_EXPLORE && phase == PH_IMPROVE) {
    ...
    Serial.println(F("No mapped path to the goal - exploring instead"));
  }
...
void setup() {
  ...
  Serial.println(speedRunReady() ? F("Press = SPEED RUN, long press = explore again.")
                                 : F("Press = EXPLORE."));
}
