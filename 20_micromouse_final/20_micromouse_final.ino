...
// ║                                                                  ║
// ║  Flood fill with turn costs: the flood works on (cell, heading), ║
// ║  so a turn costs more than going straight on. The robot keeps    ║
// ║  going forward instead of taking a turn that gains nothing, and  ║
// ║  the speed run takes the route with fewer turns.                 ║
// ║   * flood fill to the goal, then back to the start, both         ║
// ║     treating unseen walls as open, so the way back tries other   ║
// ║     routes (as ukmars/mazerunner-core),                          ║
// ║   * a wall is mapped the first time it is seen; after that only  ║
// ║     a clear-cut ToF reading can change it, so a borderline       ║
// ║     reading can never make the map flip back and forth,          ║
...
// ║   * Every stop: the front ToF looks down the corridor; open      ║
// ║     space one or two cells ahead is put in the map, so the flood ║
// ║     knows the corridor goes on.                                  ║
// ║   * A clear-cut reading that disagrees with the map is read      ║
// ║     again and, if both agree, corrects the map (e.g. a false     ║
// ║     wall that would make the robot turn early).                  ║
...
#define EEPROM_MAGIC        (0x60 ^ MAZE_SHORT ^ (MAZE_LONG << 3) ^ GOAL_SIZE)
...
// This section is compiled on a PC by tests/micromouse_20_sim.sh and run on random mazes.
