// ╔══════════════════════════════════════════════════════════════════╗
// ║  14_floodfill_final                                              ║
// ║  Flood-fill micromouse for a 99 x 126 mm robot whose axle is     ║
// ║  47 mm behind the front.                                         ║
// ║                                                                  ║
// ║  Button (START):                                                 ║
// ║    short press = SEARCH: explore to the goal, then back home     ║
// ║    long press (1 s, LED on) = FAST RUN on the known map          ║
// ║    held while powering on = forget the saved map                 ║
// ║    press during a search = give up and drive home                ║
// ║  The map is saved to EEPROM at the goal and at home, so a fast   ║
// ║  run still works after switching off.                            ║
// ║                                                                  ║
// ║  Movement:                                                       ║
...
// right. The goal is the GOAL_SIZE x GOAL_SIZE square whose lower-left cell is (GOAL_X, GOAL_Y).
#define MAZE_SIZE   16     // 16 for a full maze; smaller for a practice maze (max 16)
#define GOAL_X       7
#define GOAL_Y       7
#define GOAL_SIZE    2     // 2 = the centre 2x2 of a 16x16 maze, 1 = a single goal cell
#define START_X      0
#define START_Y      0
...
#define EEPROM_MAGIC        (0xB0 ^ MAZE_SIZE)
...
#if MAZE_SIZE > 16
#error "MAZE_SIZE must be 16 or less"
#endif
...
uint8_t dist[MAZE_SIZE][MAZE_SIZE];  // flood-fill distance to the target, 255 = unreachable
...
bool isGoal(int x, int y) {
  return x >= GOAL_X && x < GOAL_X + GOAL_SIZE && y >= GOAL_Y && y < GOAL_Y + GOAL_SIZE;
}

// Records one wall on both cells that share it. A wall, once seen, is never removed.
...
void initMaze() {
  ...
  for (int i = 0; i < MAZE_SIZE; i++) {
    setWall(i, 0, 2, true);
    setWall(i, MAZE_SIZE - 1, 0, true);
    setWall(0, i, 3, true);
    setWall(MAZE_SIZE - 1, i, 1, true);
  }
}

// pessimistic: unseen walls count as walls (fast run); otherwise as open (search).
...
void flood(bool toGoal, bool pessimistic) {
  ...
  for (int x = 0; x < MAZE_SIZE; x++) {
    for (int y = 0; y < MAZE_SIZE; y++) {
      if (toGoal ? isGoal(x, y) : (x == START_X && y == START_Y)) {
        dist[x][y] = 0;
        queue[tail++] = x * MAZE_SIZE + y;
      }
    }
  }
  while (head < tail) {
    uint8_t c = queue[head++];
    int x = c / MAZE_SIZE, y = c % MAZE_SIZE;
    for (uint8_t d = 0; d < 4; d++) {
      if (blocked(x, y, d, pessimistic)) continue;
      int nx = x + DX[d], ny = y + DY[d];
      if (!inMaze(nx, ny) || dist[nx][ny] != 255) continue;
      dist[nx][ny] = dist[x][y] + 1;
      queue[tail++] = nx * MAZE_SIZE + ny;
    }
  }
}
...
int driveStraight(float distMm, int maxPwm, bool center, bool stopAtWall, float startAxleMm) {
  ...
  lastOvershootMm = moved + corr - goal;
  return result;
}
...
// Reads the walls of the current cell from its decision point.
void senseWallsHere() {
  float l, f, r;
  averageAll(l, f, r);
  setWall(posX, posY, facing, f < FRONT_WALL_THRESHOLD_MM);
  setWall(posX, posY, (facing + 1) & 3, r < WALL_THRESHOLD_MM);
  setWall(posX, posY, (facing + 3) & 3, l < WALL_THRESHOLD_MM);
}

void saveMaze() {
  EEPROM.update(0, EEPROM_MAGIC);
  const uint8_t *p = &maze[0][0];
  for (int i = 0; i < MAZE_SIZE * MAZE_SIZE; i++) EEPROM.update(1 + i, p[i]);
}

bool loadMaze() {
  if (EEPROM.read(0) != EEPROM_MAGIC) return false;
  uint8_t *p = &maze[0][0];
  for (int i = 0; i < MAZE_SIZE * MAZE_SIZE; i++) p[i] = EEPROM.read(1 + i);
  return true;
}

// ASCII map with the robot's position (^ > v <) and unseen walls as '.'.
void printMaze() {
...
void reportPath() {
  flood(true, false);
  uint8_t optimistic = dist[START_X][START_Y];
  flood(true, true);
  uint8_t known = dist[START_X][START_Y];
...
void finishAtHome() {
  ...
  printMaze();
  reportPath();
  Serial.println(F("\nHOME. Short press = search again, long press = fast run."));
  runState = ST_WAIT;
}
...
void searchStep(bool toGoal) {
  ...
  if (dist[posX][posY] == 255) {
    // A misread wall has closed every route. Start the map again (keeps searching).
...
void fastStep() {
  flood(true, true);
  if (dist[posX][posY] == 255) {
    Serial.println(F("No fully known path - do a search first (short press)."));
    runState = ST_WAIT;
    return;
  }
  if (dist[posX][posY] == 0) {
  ...
  facing = d;
  if (runState == ST_WAIT) return;
  noteResult(moveCells(n, SPEED_RUN_PWM));
  posX += n * DX[d]; posY += n * DY[d];
}
...
void setup() {
  ...
  Serial.println(F("Short press = SEARCH, long press = FAST RUN."));
}
