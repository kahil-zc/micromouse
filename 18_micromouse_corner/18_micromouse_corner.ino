// ╔══════════════════════════════════════════════════════════════════╗
// ║  18_micromouse_corner                                            ║
// ║  Flood-fill micromouse for a 10-across x 5-ahead maze of 18 cm   ║
// ║  cells. The goal is the corner diagonally opposite the start.    ║
...
// ║    1st press  = SEARCH: flood fill to the opposite corner,       ║
...
// ║   * Every stop: the walls are read, even in mapped cells. A      ║
// ║     reading that disagrees with the map is read a second time    ║
...
// right. The goal is the corner diagonally opposite the start: the far row, at the other side.
#define MAZE_W  10   // cells across (left-right as the robot starts)
#define MAZE_H   5   // cells ahead
...
#define EEPROM_MAGIC        (0xA0 ^ MAZE_W ^ (MAZE_H << 4) ^ GOAL_SIZE)
...
#endif
#if GOAL_SIZE < 1 || GOAL_SIZE > 3
#error "GOAL_SIZE must be 1, 2 or 3"
#endif
...
// This section is compiled on a PC by tests/micromouse_18_sim.sh and run on random mazes.
// Directions: 0 = ahead (+y), 1 = right (+x), 2 = back, 3 = left, as seen from the start.
// Right of d is (d + 1) & 3.
#define PH_SEARCH   0   // flood fill to the goal, mapping every cell on the way
#define PH_IMPROVE  1   // at the goal: mapping only cells that could give a shorter path
...
// Goal: the GOAL_SIZE x GOAL_SIZE block in the far corner on the other side from the start.
bool isGoal(int8_t x, int8_t y) {
  int8_t gx = (startX == 0) ? MAZE_W - GOAL_SIZE : 0;
...
uint8_t plan(uint8_t &dir) {
  ...
  switch (phase) {
    case PH_SEARCH:
      if (isGoal(posX, posY)) { phase = PH_IMPROVE; newCells = 0; return 0; }
      targetGoal();
    ...
    case PH_FAST:
      ...
      if (noRoute) { noRoute = false; phase = PH_SEARCH; }  // map incomplete: search instead
      return n;
    case PH_RETURN:
      if (isStart(posX, posY)) { phase = PH_DONE; return 0; }
      targetStart();
...
void recoverRoute() {
  ...
  // Start the map again. Still in the start column after deciding it started in the right-hand
  // corner? That may be the misreading, so forget it too (it is found again if it was right).
...
void sense() {
  updateGyro();
  updateToF();
  if (runState == ST_RUN && (phase == PH_SEARCH || phase == PH_IMPROVE) && digitalRead(START_BUTTON) == LOW)
...
// Reads the three walls of the current cell from its decision point. Done in
...
bool loadMaze() {
  ...
  return startX == 0 || startX == MAZE_W - 1;
}
...
// Forgets the saved map and start corner.
void clearMap() {
  initMaze();
...
void runStep() {
  ...
  if (abortExploration && (phase == PH_SEARCH || phase == PH_IMPROVE)) {
    abortExploration = false;
    phase = PH_HOME;
  }
  ...
  if (phase != oldPhase) {
    if (oldPhase == PH_SEARCH && phase == PH_IMPROVE) { saveMaze(); blink(5); }  // goal reached
    if (oldPhase == PH_IMPROVE && phase == PH_HOME) saveMaze();
