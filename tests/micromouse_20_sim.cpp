// Runs the maze section of 20_micromouse_final.ino on random 5 x 10 mazes.
//   tests/micromouse_20_sim.sh
// The script copies the lines between "// --- MAZE (no hardware calls) ---" and
// "// --- END MAZE ---" into maze_20_section.inc, so this tests the exact planner the robot runs.
//
...
// Turns are counted to compare the turn-cost flood with a plain cell-count flood.
//
...
// Noisy readings: every wall reading is wrong with some chance. A wrong reading is a borderline
// one, so it is never "clear-cut"; right readings are. A wall the robot drives at stops it (the
// front ToF). Fails if it loops or never gets home.
...
static uint8_t lastSure;
static uint8_t readWalls() {
  uint8_t w = 0;
  lastSure = 0;
  for (uint8_t i = 0; i < 3; i++) {
    int d = (facing + 3 + i) & 3;
    bool t = trueWallAt(posX, posY, d);
    bool r = readWall(posX, posY, d);
    if (r) w |= 1 << i;
    if (r == t) lastSure |= 1 << i;  // wrong readings are borderline, never clear-cut
  }
  return w;
}
static int fixes = 0;
static void senseHere() {
  ...
  uint8_t sure = lastSure;
  if (showsRightCorner(walls)) walls |= readWalls();
  else if ((walls ^ mapWalls()) & seenWalls() & sure) {
    uint8_t walls2 = readWalls();
    sure &= lastSure & ~(walls ^ walls2);
  }
  uint8_t before = corrections;
  recordWalls(walls, sure);
  fixes += corrections - before;
  if (!(walls & 2)) {  // the front ToF down the corridor (true walls)
    uint8_t k = 0;
    int x = posX, y = posY;
    while (k < 2) {
      x += DX[facing]; y += DY[facing];
      if (trueWallAt(x, y, facing)) break;
      k++;
    }
    if (!trueWallAt(posX, posY, facing)) recordCorridor(k);
  }
}

static bool reachedGoal;
static long straightRuns[20];  // search: cells driven straight without turning, by length
static long searchTurns = 0, fastTurns = 0;
static int resets = 0;
static int run(uint8_t firstPhase) {
  ...
  for (int steps = 0; steps < 3000; steps++) {
    ...
    if (dir != facing) { searchTurns++; if (straight) { straightRuns[straight < 19 ? straight : 19]++; straight = 0; } }
    facing = dir;
    uint8_t done = 0;
    for (; done < n; done++) {
...
int main() {
  ...
  printf("  turns: search %.1f per maze, speed run %.2f per maze\n", (double)searchTurns / N, (double)fastTurns / N);
  ...
  for (int k = 0; k < 2; k++) {
    ...
    fails += stuck;
    printf("  %.0f%% OF WALL READINGS WRONG: %d stuck; reached the real goal %d / %d, speed run reached it %d; %d map resets, %d map corrections\n",
           noise * 100, stuck, goalOk, N, fastOk, resets, fixes);
  }
  return fails ? 1 : 0;
}
